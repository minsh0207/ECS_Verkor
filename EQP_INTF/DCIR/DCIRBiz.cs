...
namespace EQP_INTF.DCIR
{
    public partial class DCIR : UserControl
    {
        ...
        private void FMSSequenceProcessEndResponseTimeOut()
        {
            try
            {
                ...
                //_jsonEcsApiMasterRecipeResponse RecipeDataFromOPCUA = (_jsonEcsApiMasterRecipeResponse)MesResponseWaitItem.parameters[0];
                List<string> TrayList = (List<string>)MesResponseWaitItem.parameters[0];    // 20230331 msh : index 오류 1 -> 0
                _CellInformation CellInfo = (_CellInformation)MesResponseWaitItem.parameters[1];
                string EQPType = (string)MesResponseWaitItem.parameters[2];
                string EQPID = (string)MesResponseWaitItem.parameters[3];
                string UNITID = (string)MesResponseWaitItem.parameters[4];
            ...
            try
            {
                ...
                if (eventItem != null)
                {
                    ...
                    if (bProcessEnd)
                    {
                        ...
                        TrayList.Add(TrayId);
                        //_CellInformation CellInfo = EQPClient.ReadProcessDataEQP("Location1.TrackOutCellInformation", "Location1.TrayInformation");
                        // 20230404 msh : ProcessId는 GetMasterNextProcessInfo에서 가져온다.
                        _CellInformation CellInfo = EQPClient.ReadProcessDataEQP("Location1.TrackOutCellInformation");
                        ...
                        // 20230328 msh : TrackOutCellInformation의 Cell 정보
        ...
        public _next_process GetMasterNextProcessInfo(out string outProcessId)
        {
            try
            {
                ...
                outProcessId = ProcessId;       // 20230324 msh : ProcessId 추가
            ...
            catch (Exception ex)
            {
                ...
                outProcessId = string.Empty;      // 20230324 msh : ProcessId 추가
                ...
                if (eventItem != null)
                {
                    ...
                    if (bProcessStart)
                    {
                        ...
                        else
                        {
                            ...
                            // 20230331 msh : TrayId를 List<string> 구조로 변경
                            List<string> trayList = new List<string>
                            {
                                TrayId
                            };
                        ...
                        if (TrayId != null && TrayId.Length > 0)
                        {
                            ...
                            if (TrayData != null && TrayData.DATA.Count > 0)
                            {
                                ...
                                if (EQPClient.WriteNodeByPath("Location1.TrackInCellInformation.CellCount", (UInt16)TrayCellData.DATA.Count) == false)
                                {
                                    ...
                                    EQPClient.WriteFMSTrouble("FmsStatus.Trouble", true, Global.FMSTrouble.OPCUA_WriteError);
                                    return;
                                }
