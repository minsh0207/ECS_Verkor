...
namespace EQP_INTF.LeakCheck
{
    public partial class LeakCheck : UserControl
    {
        ...
        public bool WaitMesResponse(string ResponsePath, List<object> parameters = null)
        {
            ...
            {
                _LOG_($"[{this.Name}] MesResponseWaitItem is not null :[{MesResponseWaitItem.ResponsePath}:{MesResponseWaitItem.RequestTime}] ", ECSLogger.LOG_LEVEL.INFO);
        ...
        private void MES_Response_TimeOut(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            if (MesResponseWaitItem.ResponsePath == "Location1.TrayProcess.TrayLoadResponse")
                FMSSequencePickTrayLoadResponseTimeOut("Location1");
            else if (MesResponseWaitItem.ResponsePath == "Location1.TrayProcess.RequestRecipeResponse")
                FMSSequencePickRequestRecipeResponseTimeOut("Location1");
            else if (MesResponseWaitItem.ResponsePath == "Location1.TrayProcess.ProcessStartResponse")
                FMSSequencePickProcessStartResponseTimeOut("Location1");
            else if (MesResponseWaitItem.ResponsePath == "Location1.TrayProcess.ProcessEndResponse")
                FMSSequencePickProcessEndResponseTimeOut("Location1");
            if (MesResponseWaitItem.ResponsePath == "Location2.TrayProcess.TrayLoadResponse")
                FMSSequencePickTrayLoadResponseTimeOut("Location2");
            else if (MesResponseWaitItem.ResponsePath == "Location2.TrayProcess.RequestRecipeResponse")
                FMSSequencePickRequestRecipeResponseTimeOut("Location2");
            else if (MesResponseWaitItem.ResponsePath == "Location2.TrayProcess.ProcessStartResponse")
                FMSSequencePickProcessStartResponseTimeOut("Location2");
            else if (MesResponseWaitItem.ResponsePath == "Location2.TrayProcess.ProcessEndResponse")
                FMSSequencePickProcessEndResponseTimeOut("Location2");
        ...
        private void AddEQPBrowsePath(OPCUAClient.OPCUAClient Client)
        {
            ...
            for (int cellNo = 0; cellNo < 30; cellNo++)
            {
                foreach (var location in locationList)
                {
                    ...
                    {
                        string browsename = $"{location}.TrackInCellInformation.Cell._{cellNo}.{item}";
        ...
        private void AddRecipeBrowsePath(OPCUAClient.OPCUAClient Client, MappingDirection mappingDirection)
        {
            ...
            foreach (var location in locationList)
            {
                ...
                {
                    browseName = string.Format($"{location}.Recipe.{item.Key}");
        ...
        private void AddProcessDataBrowsePath(OPCUAClient.OPCUAClient Client, MappingDirection mappingDirection)
        {
            ...
            foreach (var location in locationList)
            {
                ...
                {
                    TagName = $"{location}.TrackOutCellInformation.Cell._";
                }
                else
                {
                    TagName = $"{location}.ProcessData.Cell";
                }
