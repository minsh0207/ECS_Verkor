...
namespace EQP_INTF.Packing
{
    public partial class Packing : UserControl
    {
        ...
        private void MES_Response_TimeOut(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            else if (MesResponseWaitItem.ResponsePath == "Location2.TrayProcess.PalletLoadResponse")
                FMSSequencePlacePalletLoadResponseTimeOut();
        ...
        private void AddEQPBrowsePath(OPCUAClient.OPCUAClient Client)
        {
            ...
            //TrayInformation
            browsePathList.Add("PickLocation.TrayInformation.TrayType");
            ...
            foreach (var location in locationList)
            {
                ...
                {
                    string browsename = $"{location}.CellInformation.Cell._{cellNo}";
                    ...
                    {
                        browsePathList.Add($"{browsename}.Floor");
                        browsePathList.Add($"{browsename}.Position");
                    }
                }
            }
        ...
        private void AddFMSBrowsePath(OPCUAClient.OPCUAClient Client)
        {
            ...
            //PalletProcess
            browsePathList.Add($"Location2.PalletProcess.PalletLoad");
            browsePathList.Add($"Location2.PalletProcess.ProcessEnd");
            ...
            foreach (var location in locationList)
            {
                ...
                {
                    browsePathList.Add($"{location}.TrayInformation.CellCount");
                    browsePathList.Add($"{location}.TrayInformation.LotId");
                    browsePathList.Add($"{location}.TrayInformation.Model");
                    browsePathList.Add($"{location}.TrayInformation.ProcessId");
                    browsePathList.Add($"{location}.TrayInformation.RouteId");
                    browsePathList.Add($"{location}.TrayInformation.TrayExist");
                    browsePathList.Add($"{location}.TrayInformation.TrayId");
                    browsePathList.Add($"{location}.TrayInformation.TrayType");

                    TagName = "TrayInformation";
                    cellCount = 30;
                }
                else
                {
                    browsePathList.Add($"{location}.PalletInformation.CellCount");
                    browsePathList.Add($"{location}.PalletInformation.LotId");
                    browsePathList.Add($"{location}.PalletInformation.PalletExist");
                    browsePathList.Add($"{location}.PalletInformation.PalletGrade");
                    browsePathList.Add($"{location}.PalletInformation.PalletId");
                    browsePathList.Add($"{location}.PalletInformation.ProductModel");
                    browsePathList.Add($"{location}.PalletInformation.RouteId");

                    TagName = "PalletInformation";
                    cellCount = 150;
                }
                ...
                {
                    if (location == "Location1")
                        browsename = $"{location}.{TagName}.CellInformation.Cell{string.Format("{0:D2}", cellNo + 1)}";
                    else
                        browsename = $"{location}.{TagName}.CellInformation.Cell{string.Format("{0:D3}", cellNo + 1)}";
                    ...
                    {
                        browsePathList.Add($"{browsename}.CellGrade");
                        browsePathList.Add($"{browsename}.Floor");
                        browsePathList.Add($"{browsename}.Position");
                    }
                }
        ...
        private void AddRecipeBrowsePath(OPCUAClient.OPCUAClient Client, MappingDirection mappingDirection)
        {
            ...
            {
                browseName = string.Format($"{TagName}.Recipe.{item.Key}");
        ...
        private void AddProcessDataBrowsePath(OPCUAClient.OPCUAClient Client, MappingDirection mappingDirection)
        {
            ...
            if (mappingDirection == MappingDirection.EqpToFms)
            {
                TagName = "PlaceLocation.CellInformation.Cell._";
            }
            else
            {
                TagName = "Location2.ProcessData.Cell";
            }
